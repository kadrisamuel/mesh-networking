...
Cryptographic operations, labels, and keys are defined in `CRYPTOGRAPHY_V1.md`. Golden bytes are in `vectors/v1/vectors.json`. Draft.3's final independent review at commit `fd871a38e0002ca7c878678a07d05489051d47fd` failed; later corrections remain draft candidates until independently reviewed and human-approved.
...
Steps 1–9 are the complete peer-visible relay-admission path and execute identically whether the routing tag is unknown, known with invalid ciphertext, or known with valid ciphertext. A known-route authentication failure leaves the committed object as an ordinary opaque relay row until its existing monotonic expiry or uniform eviction. Route knowledge and every authentication/decryption/private-processing result MUST NOT change the queued ACK, offered-entry or byte counters, session limits or quota decision, inventory visibility/order, forwarding eligibility, retention deadline, stored hops/PoW form, GOODBYE/error behavior, session response or closure, or any other peer-visible behavior. Neither route knowledge nor any authentication/decryption result may be stored in the relay database. Successful private processing records its idempotency/delivery state only in the private database and MUST NOT remove, reprioritize, or mutate the relay copy. A crash after relay commit but before ACK transmission is recovered by the peer's byte-identical retry, which receives canonical-duplicate status.
...
For every Noise session and direction, regardless of route knowledge or any authentication/decryption/private-processing result, accept at most 256 offered envelope entries and 1,048,576 total envelope bytes, whichever is reached first. Duplicates count toward offered entries and bytes so they cannot bypass work limits. The session closes at 60 seconds regardless of progress. Maximum concurrent sessions are four on mobile and sixteen on desktop; excess peers receive only a generic busy close.
