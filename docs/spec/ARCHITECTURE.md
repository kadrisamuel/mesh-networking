...
- Each sender/epoch outer key has a durable `2^24`-seal budget. Ordinary sealing stops at `2^24 - 1`; the final old-key seal is reserved for one self-Update envelope. One private transaction stores that immutable old-epoch envelope, merges the new MLS epoch, persists its exporters and zero counter, and is committed before emission. Post-commit retries reuse the stored bytes; an incoming valid epoch transition may supersede an uncommitted local rollover.
...
- security-sensitive implementation remains blocked until a new independent review resolves and accepts all findings from the preserved failed draft.1, draft.2, and draft.3 reviews and a human reviewer approves the corrected composition and reproduced v1 vectors;
